...
function edit_obj()
{
    location.href='/register/artCar_reg.php?_id='+get_id_for_event(this)+'&is_admin=true';
}
...
function del_obj()
{
    ...
    bootbox.confirm("Are you sure you want to delete this art car?", really_del);
}
...
{
    $.ajax({
        url: '/register/api/v1/dmv',
        success: data_obtained
    });
    $('#dmv tbody').on('click', 'button[name="edit"]', edit_obj);
    $('#dmv tbody').on('click', 'button[name="del"]', del_obj);
}
